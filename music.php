...
<body onload="initMusic()" id="get_token" class="">
...
$parentcatid=isset($_GET['pcatid'])?$_GET['pcatid']:'';
 ...
 <div class="container fullHeight">
  ...
  <!-- Tab Buttons-->
<div class="w3-row header header-menu barSubItems1">
...
function buildMusicTabItems(JSONobject)
    {
        ...
        if(len>0)
        {
        	...
        	if(Parent_id==pcatid){ getDataWithCategory(Parent_id,"music-result"); $("#load_more_button").hide();
        ...
        for(var i = 0; i < len ; i++)
        {
         ...
         if(pcatid=='' && i==0)
         { var cl="active-bar"; }
         tab+='<div class="baritem-container"><span class="bar-item '+cl+'" id="TAB'+Parent_id+'" onclick=callTab(\''+Parent_id+'\',\''+Parent_name+'\') >'+Parent_name+'</span></div>';
        }
        tab+='</center>';
        $("#button_load").show();
        var activeId="";
...
function callTab(cid,cname)
{
    ...
    document.location = baseUrl+"/music/"+ncname+"/"+cid;
    //return false;
}
