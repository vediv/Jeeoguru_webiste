...
function buildMenu(JSONobject, Container,pagename,loginstatus,userid)
{
    ...
    for ( var i = 0; i < items ; i++)
    {
       ...
       // alert(menu_icon);
        ...
        else if((viewType==5 && catname=='MAKERS') || (viewType=="5" && catname=='MAKERS'))
        {
            url=baseUrl+"/makers";
            sessionStorage.setItem("maker_id",catID);
        }
...
function loginArea()
{
    switch(loginStatus)
    {
        ...
        default:
             var logint="log_in";
             return '<li class="nav-menu w3-right   float-none"><a  href="javascript:" onclick="login()"  class="padding-login pad-l">LOGIN</a></li>';
            break;
    }
...
function buildCaruselonly(JSONobject, Container)
{
    ...
    control+='<div class="w3-center  w3-large w3-text-white  w3-display-middle1 slideControls" style="width:100%">';
    control+='<div class="w3-left  arrowL"  onclick="slider(-1)" style="cursor:pointer;"></div>';
    control+='<div class="w3-right  arrowR" onclick="slider()" style="cursor:pointer;"></div>';
    control+='</div>';

    control+='<div class="w3-center   w3-large w3-text-white  " style="width:100%; margin-top: 1px !important;">';
    for (var i = 0; i <items_carosel ; i++)
    {
        control += '<span style="cursor:pointer;" class="w3-badge demo  w3-transparent w3-hover-white" onclick="slider('+i+')"></span>';
    }
  ...
  function buildCategoryWithDatafilm(JSONobject){
        ...
        control='<div class="w3-display-container w3-row-padding" style="margin-top:7px">';
...
function buildKidsCategory(JSONobject, Container){
 ...
 function buildKidsCategory(JSONobject, Container){
       ...
       function buildBazarCategory(JSONobject, Container){
      	var control='<div class=" w3-row-padding" style="margin-top:30px">';
      	var Category_data_len= JSONobject.Category_data.length;
      	var entryid = JSONobject.Category_data.entryid;
        for(var i=0; i < Category_data_len; i++)
         {
...
function buildcategory(JSONobject,Container)
{
	 var control='';
	  if(Container=='category-home'){
	     control+='<div class=" w3-text-white" style="">';
	     //control+='<div class="carousel-inner">';
	     ...
	     for(var i=0; i < catinfo_len; i++)
	     {
		    ...
		    // alert(Parent_id);
		     var tumnail_height_width="/width/500/height/300/quality/100";
             var thumburl=imgurl+tumnail_height_width;
             console.log(thumburl);
		     control+='<div class="w3-third  w3-center w3-row-padding " style="margin-top: 2%; position:relative">';
		   ...
		   //  control+='<div class="w3-display-middle w3-display-middle-new"  style="border-bottom: 2px solid #3498db;"><a href="series/'+Parent_id+'"><h6 style="font-weight: 700 !important;" title="'+name+'">'+Parent_name+' </h6></a> </div> <br />';
		     }
		     if(sub_count==0){
		     	console.log("count="+sub_count);
		    ...
		    // control+='<div class="w3-display-middle w3-display-middle-new"  style="border-bottom: 2px solid #3498db;"><a href="episode/'+Parent_id+'" style="cursor:pointer"><h6 style="font-weight: 700 !important;" title="'+name+'">'+Parent_name+' </h6></a> </div> <br />';
		     }
		     control+='</div>';
	     }
	     control +='</div>';
	 ...
	 else if(Container=='category-image')
	 {
		...
		for(var i=0; i < catinfo_len; i++)
	      {
		     ...
		     var Parent_id = JSONobject.category_info[i].Parent_id;
		     var imgurl =JSONobject.category_info[i].t_original_url;
		     var tumnail_height_width="/width/300/height/200/quality/100";
             var thumburl=imgurl+tumnail_height_width;
		     if(pat_id==Parent_id ){
		      control+='<div class="w3-display hero-image  relative">';
		      control+='<img class="bgimgOrg" style=" "  src="'+imgurl+'" >';
		      control+='<h1 class="w3-text-white text-shadow w3-bold w3-display-middle font-size" title="'+name+'"></h1>'; }     //'+Parent_name+'
		      control+='</div>';
	       }
      ...
      {
          var control='<div class="w3-row-padding">';   //w3-display-container
       ...
       // control+='<a href="../episode/'+Parent_id+'"><h5 class="font-size " title="'+vname+'" style="padding-left:2px; text-align: left;">'+vname_vew+'</h5></a>';
        control+='</div>';
        control+='</div>';
        control+='</div>';
     ...
     function build_episode(JSONobject,Container)
      {
          var CategoryLen= JSONobject.Category_data.length;
          var control='<div class=" w3-row-padding">';      //w3-display-container

          if(CategoryLen<=0){ _(Container).innerHTML="<center><span class='w3-prussian_blue' style='margin-top:10%;font-size:19px;'> &nbsp;No videos in continue watching.. &nbsp;<span></center>";return false;}
          //var entryid = JSONobject.Category_data.entryid;
        ...
        control+='<span class="'+planClass+'" style="bottom: 27%; z-index: 1; position: absolute;">'+plandetail+'</span>';
        control+='<div class="opacity w3-text-white">';
        control+='<a href="javascript:" onCLick="viewMedia(\''+entryid+'\',\''+plan+'\')"><h5 class="font-size " title="'+vname+'" style="padding-left:2px; text-align: left;">'+vname_vew+'</h5> </a>';
        control+='</div>';
        control+='</div>';
        control+='</div>';
...
function buildMyProflie(JSONobject, Container)
 ...
 function buildUserHistory(JSONobject, Container)
    ...
    function buildMoviesHtml(JSONobject, Container)
   {
	if(Container=='movies-result'){
	     ...
	     //control+='<div class="carousel-inner">';
	     var catinfo_len= JSONobject.category_info.length;
	      if(catinfo_len<=0){_(Container).innerHTML ="<center><span class='w3-prussian_blue' style='margin-top:10%;font-size:19px;'> &nbsp;No videos available.. &nbsp;<span></center>";return false;}
         ...
         control+='<a href="moviesall/'+Parent_id+'" class=""><img class="img-res zoom"     data-src="'+baseUrl+'/images/banners/placeholder@.png" src="'+imgurl+'" style="box-shadow: 3px 3px 20px rgba(0, 0, 0, .5);"> ';

        // control+='<div class="w3-display-middle w3-display-middle-new"  style="border-bottom: 2px solid #3498db;"><h6 style="font-weight: 700 !important;" title="'+name+'">'+Parent_name+'</h6> </div> </a><br />';
		...
		for(var i=0; i < catinfo_len; i++)
	     {
			//	console.log(JSONobject.category_info[i]);
		     ...
		     control+='<h4 class=" w3-jumbo1 w3-display font-size script" ></h4>';}   //'+Parent_name+'
		     //control+='<hr class="w3-border-grey" style="margin:auto;">';
		     control+='</div>';
	     }
 ...
 {
 	       var control='<div class=" w3-text-white " style="margin-left: 20px; ">';  //padding-200 w3-display-container
      	 var Category_data_len= JSONobject.Category_data.length;
      	  if(Category_data_len<=0){_(Container).innerHTML ="<center><span class='w3-prussian_blue' style='margin-top:10%;font-size:19px;'> &nbsp;No videos available.. &nbsp;<span></center>";return false;}
        ...
        control+='<div class="w3-col l2 m6 s12  w3-center w3-row-padding lazy-container" style="margin-top: 0%; position:relative">';

        control+='<div class="w3-card-2">';
        control+='<a style="cursor:pointer;"><img class="lazy img-res-mov" src="'+thumburl+'"  onCLick="viewMedia(\''+entryid+'\',\''+plan+'\')" style="" ></a>';
        control+='<span class="duration">'+msToTime(duration)+'</span>';
         control+='<span class="'+planClass+' " style="bottom: 2%;  z-index: 1; position: absolute;">'+plandetail+'</span>';
        //control+='<div class="w3-display-middle w3-large"  style="border-bottom: 2px solid #3498db;">';
        control+='<div class="opacity w3-text-white ">';
        //control+='<a onCLick="viewMedia(\''+entryid+'\',\''+plan+'\')" style="cursor:pointer"> <h5 class="font-size" title="'+name+'"  style="  padding-bottom:0px !important; text-align: left;white-space: nowrap;width: 85%;overflow: hidden; text-overflow: ellipsis;">'+name+'</h5></a>';
        control+='</div>';
        control+='</div>';
        control+='</div>';
        //control+='</div>';
